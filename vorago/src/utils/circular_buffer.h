...
} VOR_CircularBuffer_t;

/**
 \fn          void VOR_CircularBuffer_Initialize(VOR_CIRCULAR_BUFFER *pBuffer, void *pData, uint16_t
 item_size, uint16_t max_items) \brief       Initilize circular buffer. \param[in]   pBuffer
 Pointer to circular buffer access structure \ref VOR_CIRCULAR_BUFFER
 \param[in]		pData				Pointer to buffer where all items are stored
 \param[in]		item_size		Specifies the size of each item in bytes
 \param[in]		max_items 	Specifies the maximum number of items that this buffer will
 hold \return      none
*/
void VOR_CircularBuffer_Initialize(VOR_CircularBuffer_t *pBuffer, void *pData, uint16_t item_size,
                                   uint16_t max_items);
...
/**
 \fn				 int32_t VOR_CircularBuffer_Write(VOR_CIRCULAR_BUFFER *pBuffer,
 const void *pItem) \brief 		 Write specified item to buffer at location pointed by write
 index. \param[in]	 pBuffer Pointer to circular buffer access structure \ref
 VOR_CIRCULAR_BUFFER \param[in]  pItem 	 Pointer to new item that will be added to circualr buffer
...
/**
 \fn				 int32_t VOR_CircularBuffer_Read(VOR_CIRCULAR_BUFFER *pBuffer, void
 *pItem) \brief 		 Read an item at current read index. \param[in]	 pBuffer Pointer to
 circular buffer access structure \ref VOR_CIRCULAR_BUFFER \param[out] pItem 	 new item retrieved
 from circualr buffer \return		 -1 if read fails because circular buffer is empty
*/
