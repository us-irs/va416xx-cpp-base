...
#define IOCFG_REG_DEFAULT (un_iocfg_reg_t){{0}} // reset value (0x00000000)
#define IOCFG_REG_PULLUP  (un_iocfg_reg_t){{.plevel = 1, .pen = 1}}
#define IOCFG_REG_PULLDN  (un_iocfg_reg_t){{.plevel = 0, .pen = 1}}
#define IOCFG_REG_FUN0    (un_iocfg_reg_t){{.funsel = 0}}
#define IOCFG_REG_FUN1    (un_iocfg_reg_t){{.funsel = 1}}
#define IOCFG_REG_FUN2    (un_iocfg_reg_t){{.funsel = 2}}
#define IOCFG_REG_FUN3    (un_iocfg_reg_t){{.funsel = 3}}

// Used to define end of IOcfg structure array
#define IOCFG_PINCFG_END  {.port = 0}
...
{
  en_iocfg_dir__input = 0, // double underscore intentional (for text alignment reasons)
...
extern hal_status_t HAL_Iocfg_Init(const stc_iocfg_pin_cfg_t* pinConfig); // init pins to sw default, then setup specified pins in pinConfig array
extern hal_status_t HAL_Iocfg_DeInit(void); // declocks and resets IOCONFIG peripheral
extern hal_status_t HAL_Iocfg_SetupPin(VOR_GPIO_Type* port, gpio_pin_number_t pin, en_iocfg_dir_t dir, un_iocfg_reg_t reg); // setup a pin
