...
int build_rmap_hdr(struct rmap_pkt *pkt, char *hdr, int *size) {
  ...
  if ((pkt->type == writecmd) || (pkt->type == readcmd) || (pkt->type == rmwcmd)) {
    ...
    *size = srcspalen * 4 + *size;
    ...
    hdr[pkt->dstspalen + 2] = hdr[pkt->dstspalen + 2] | (type << 6) | (write << 5) |
                              (pkt->verify << 4) | (pkt->ack << 3) | (pkt->incr << 2) | srcspalen;
    ...
    hdr[pkt->srcspalen + 2] = hdr[pkt->srcspalen + 2] | (type << 6) | (write << 5) |
                              (pkt->verify << 4) | (pkt->ack << 3) | (pkt->incr << 2) | srcspalen;
