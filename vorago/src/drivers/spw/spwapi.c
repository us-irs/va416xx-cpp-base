...
static inline int loadmem(int addr) { return *((int *)addr); }
...
int spw_init(struct spwvars *spw) {
  ...
  /* Enable pads */
  VOR_SYSCONFIG->SPW_M4_CNTL |= SYSCONFIG_SPW_M4_CNTL_SPW_PAD_EN_Msk;
  ...
  spw->pnp = ((tmp >> 18) & 3);
  if ((tmp >> 24) & 1) {
    tmp = loadmem((int)&(spw->regs->intdist.ctrl));
  ...
  spw->regs->ctrl =
      0x2 | (spw->timetxen << 10) | (spw->timerxen << 11) | (spw->port << 21); /*set ctrl*/
  for (i = 0; i < spw->dmachan; i++) {
...
void spw_disable(struct spwvars *spw) { spw->regs->ctrl = loadmem((int)&(spw->regs->ctrl)) | 0x1; }

void spw_enable(struct spwvars *spw) { spw->regs->ctrl = loadmem((int)&(spw->regs->ctrl)) & ~0x1; }

void spw_start(struct spwvars *spw) { spw->regs->ctrl = loadmem((int)&(spw->regs->ctrl)) | 0x2; }

void spw_stop(struct spwvars *spw) { spw->regs->ctrl = loadmem((int)&(spw->regs->ctrl)) & ~0x2; }
...
int spw_set_nodeadr(struct spwvars *spw) {
  if ((spw->nodeaddr < 0) || (spw->nodeaddr > 255) || (spw->mask < 0) || (spw->mask > 255)) {
    return 1;
  } else {
...
int spw_set_chanadr(int dmachan, struct spwvars *spw) {
  if ((spw->dma[dmachan].addr < 0) || (spw->dma[dmachan].addr > 255) ||
    ...
    spw->regs->dma[dmachan].addr =
        (spw->dma[dmachan].addr & 0xFF) | ((spw->dma[dmachan].mask & 0xFF) << 8);
    return 0;
  }
}
...
int spw_tx(int dmachan, int hcrc, int dcrc, int skipcrcsize, int hsize, char *hbuf, int dsize,
  ...
  if ((((hcrc == 1) || (dcrc == 1)) && ((spw->rmapcrc | spw->rmap) == 0)) || (hcrc < 0) ||
      (hcrc > 1) || (dcrc < 0) || (dcrc > 1)) {
    return 3;
  }
  ...
  if (spw->dma[dmachan].txpnt == (spw->ntxdesc - 1)) {
    spw->dma[dmachan].txd[spw->dma[dmachan].txpnt].ctrl =
        0x3000 | hsize | (hcrc << 16) | (dcrc << 17) | (skipcrcsize << 8);
    spw->dma[dmachan].txpnt = 0;
  } else {
    spw->dma[dmachan].txd[spw->dma[dmachan].txpnt].ctrl =
        0x1000 | hsize | (hcrc << 16) | (dcrc << 17) | (skipcrcsize << 8);
    spw->dma[dmachan].txpnt++;
  }

  spw->regs->dma[dmachan].ctrl = loadmem((int)&(spw->regs->dma[dmachan].ctrl)) & 0xF8C6FAAA | 1;
...
int spw_rx(int dmachan, char *buf, struct spwvars *spw) {
  ...
  spw->regs->dma[dmachan].ctrl =
      loadmem((int)&(spw->regs->dma[dmachan].ctrl)) & 0xF8C8F955 | 2 | (1 << 11);
  return 0;
}
...
int get_time(struct spwvars *spw) { return (loadmem((int)&(spw->regs->timereg)) & 0x3F); }
...
void spw_pnpen(struct spwvars *spw) {
  spw->regs->ctrl = loadmem((int)&(spw->regs->ctrl)) | (1 << 15);
}
