...
static int rmap_read(char dsta, char srca, int addr, int len, int incr, int dstkey, char *data,
                     struct spwvars *spw) {
  int k;
  int tmp;
  int iterations;
...
#if TEST3 == 1
  printf(
      "TEST 3: Tx and Rx with varying size and alignment (if possible in configuration) from/to "
      "DMA channel\n\n");
    ...
    for (m = 1; m < 4; m++) {
      ...
      for (j = 0; j < i; j++) {
        if (loadb((int)&(rxbuf[j + n])) != txbuf[j]) {
          printf("Compare error: %u Data: %x Expected: %x \n", j,
                 (unsigned)loadb((int)&(rxbuf[j + n])), (unsigned)txbuf[j]);
          exit(1);
        }
      }
#endif
  ...
  for (i = 0; i < 64; i++) {
    ...
    for (m = 0; m < 8; m++) {
      ...
      if ((((((int)&(rx0[(m % 4)])) % 2) != 0) && ((cmd->len / 2) == 2)) ||
          (((((int)&(rx0[(m % 4)])) % 4) != 0) && ((cmd->len / 2) == 4)) || ((cmd->len / 2) == 3)) {
        reply->status = 10;
      } else {
        reply->status = 0;
      }
      ...
      if (reply->status == 0) {
        for (k = *replysize + 1; k < *replysize + 1 + (i / 2); k++) {
          if (loadb((int)&(rx1[k])) != rxdata[k - *replysize - 1 + (m % 4)]) {
            printf("Compare error 2: %u Data: %x Expected: %x \n", k,
                   (unsigned)loadb((int)&(rx1[k])), (unsigned)rxdata[k - *replysize - 1 + (m % 4)]);
            exit(1);
          }
        }
        for (k = 0; k < (i / 2); k++) {
          rxdata[(m % 4) + k] =
              ((tx1[k] & tx1[k + (i / 2)]) | (rxdata[(m % 4) + k] & ~tx1[k + (i / 2)]));
        }
      }
    }
    /* if (((i % 4) == 0) && ((m % 8) == 0)) { */
...
#if TEST12 == 1
  printf(
      "TEST 12 Indefinite loop with continuous full-duplex transfers testing long time "
      "stability\n\n");
      ...
      for (k = 0; k < rxlen; k++) {
        if (loadb((int)&(rx[rxchkpnt][k])) != chk[chkpnt][k]) {
          printf("Compare error: %u %u %u Data: %x Expected: %x \n", rxchkpnt, chkpnt, k,
                 (unsigned)loadb((int)&(rx[rxchkpnt][k])), (unsigned)chk[chkpnt][k]);
        }
      }
