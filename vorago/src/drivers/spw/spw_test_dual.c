...
static inline char loadb(int addr) {
  char tmp;
  asm(" lduba [%1]1, %0 " : "=r"(tmp) : "r"(addr));
  return tmp;
}

static inline int loadmem(int addr) {
  int tmp;
  asm(" lda [%1]1, %0 " : "=r"(tmp) : "r"(addr));
  return tmp;
}
...
/*   /\************************ TEST 8 **************************************\/ */
  ...
  printf(
      "Maximum speed test started (several minutes can pass before the next output on screen)\n");
  t1 = clock();
  for (i = 0; i < 64; i++) {
  ...
  printf(
      "Maximum speed test full-duplex started (several minutes can pass before the next output on "
      "screen)\n");
  tx0[0] = 0x1;
  tx0[1] = 0x2;
  for (i = 2; i < MAXSIZE; i++) {
  ...
  for (k = 0; k < maxlen - 2; k++) {
    if (loadb((int)&(rxbuf[k + 2])) != txbuf[k]) {
      printf("Compare error: %u Data: %x Expected: %x \n", k, (unsigned)loadb((int)&(rxbuf[k + 2])),
             (unsigned)txbuf[k]);
      exit(1);
    }
  }
          ...
          for (k = 0; k < i; k++) {
            if (loadb((int)&(rx1[*replysize + 1 + k])) != tx1[k]) {
              printf("Compare error 1: %u Data: %x Expected: %x \n", k,
                     (unsigned)loadb((int)&(rx1[*replysize + 1 + k])), (unsigned)tx1[k]);
              exit(1);
            }
          }
...
/*   /\************************ TEST 12 **************************************\/ */
  ...
  if ((spw1->ver == 1) && (spw2->ver == 1)) {
    ...
    for (i = 0; i < 1048576; i++) {
       ...
       * spw2->dma[l].addr, spw2->dma[l].mask, spw2->dma[l].addr & ~spw2->dma[l].mask, destaddr &
       * ~spw2->dma[l].mask); */
      /*                   } */
      rmaprx = 0;
      if (rmappkt) {
      ...
      for (l = 0; l < spw2->dmachan; l++) {
        ...
        if (chanen[l]) {
          if (((spw2->dma[l].addr & ~spw2->dma[l].mask) == (destaddr & ~spw2->dma[l].mask)) &&
              sepaddr[l]) {
            if (!(rmaprx) && !(found)) {
              startrx[l] = 0;
              rxchan = l;
          ...
          } else if (((spw2->nodeaddr & ~spw2->mask) == (destaddr & ~spw2->mask)) &&
                     !(sepaddr[l]) && !(rmappkt)) {
            if (!(found)) {
              startrx[l] = 0;
              rxchan = l;
      ...
      /* printf("Addr: %x, Rxchan: %d, sepaddr: %d, DefAddr: %x, DefMask: %x, RxAddr: %x, RxMask:
       * %x, Rmap: %d\n", tx0[0], rxchan, sepaddr[rxchan], spw2->nodeaddr, spw2->mask,
       * spw2->dma[rxchan].addr, spw2->dma[rxchan].mask, rmappkt); */
      /*                   printf("Here\n"); */
      ...
      if (!(rmaprx) && found) {
        ...
        if (rmappkt) {
          for (l = 0; l < *cmdsize; l++) {
            if (loadb((int)&(rx[rxchan][l])) != tx1[l]) {
              printf("Compare error buf 0: %u Data: %x Expected: %x \n", l,
                     (unsigned)loadb((int)&(rx[rxchan][l])), (unsigned)tx1[l]);
              exit(1);
            }
          }
          for (l = 0; l < 128; l++) {
            if (loadb((int)&(rx[rxchan][l + *cmdsize + 1])) != tx0[l]) {
              printf("Compare error buf 0: %u Data: %x Expected: %x \n", l,
                     (unsigned)loadb((int)&(rx[rxchan][l + *cmdsize + 1])), (unsigned)tx0[l]);
              exit(1);
            }
          }
        } else {
          for (l = 0; l < length; l++) {
            if (loadb((int)&(rx[rxchan][l])) != tx0[l]) {
              printf("Compare error buf 0: %u Data: %x Expected: %x \n", l,
                     (unsigned)loadb((int)&(rx[rxchan][l])), (unsigned)tx0[l]);
              exit(1);
            }
          }
        }
        ...
        for (l = 0; l < *replysize; l++) {
          if (loadb((int)&(rx1[l])) != rx2[l]) {
            printf("Compare error buf 0: %u Data: %x Expected: %x \n", l,
                   (unsigned)loadb((int)&(rx1[l])), (unsigned)rx2[l]);
            exit(1);
          }
        }
      }
